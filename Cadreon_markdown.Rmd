...
# Clean up the labels and scale the values
salesData <- salesData %>% 
  mutate(digital_spend = digital_spend/10^6) %>% 
  mutate(sales = sales/10^6) %>% 
  mutate(tv_spend = tv_spend/10^6) %>% 
  rename(`Digital Sales ($Mil)` = digital_spend) %>%
  rename(`Total Sales ($Mil)` = sales) %>%
  rename(`TV Sales ($Mil)` = tv_spend)
...
```{r q3, echo=FALSE}
features <- c("Digital Sales ($Mil)", "Total Sales ($Mil)", "TV Sales ($Mil)")
...
currentTriangleFunction <- function(data, mapping, color = I("grey50"), sizeRange = c(3, 5), ...) {
...
```
Sales is positively correlated with both types of investment. TV spending and digital spending do have some positive correlation, however is fairly small and not statistically significant at the 0.05 threshold. This is sufficient to say that their relationship is unlikely to cause a multicolinearity problem. 
...
##Question 7
We simple need to feed in the values to our model.
```{r q7, echo=FALSE}
